...
def sumOfCustos(custos):
    custosSum= []
    for r in range(len(custos)):
        custosSum.append([0]+custos[r])
    return custosSum


def findSums(custosSum,dinheiro,numeroDeUnidades,i=0,result=0, nestedList = None,A=[]):
    if i == 0:
        nestedList = []
    if i>=numeroDeUnidades:
        ...
        for r in range(len(custosSum[i])):
            ...
            if result> dinheiro:
                ...
                result, A = findSums(custosSum,dinheiro, numeroDeUnidades,i=i+1,result=result, nestedList=nestedList,A=A)
    return result, A 
...
indicesPossiveisPrecosPorUnidade = findSums(custosSum,dinheiro, numeroDeUnidades)[1]
